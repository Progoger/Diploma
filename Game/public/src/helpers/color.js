export default class Color {
    constructor(scene, place, color) {
        ...
        this.render = (x, y, sprite) => {
            let color = scene.add.image(x, y, sprite).setScale(0.14, 0.14).setInteractive();
            color.on('pointerdown', function (event) {
                ...
                if (newAppereance != this.scene.appereance){
                    ...
                    this.scene.character = this.scene.add.image(innerWidth*0.275, innerHeight*0.5, this.scene.appereance).setScale(0.14, 0.14).setInteractive();
                }
            }, this);
            return color;
        }
