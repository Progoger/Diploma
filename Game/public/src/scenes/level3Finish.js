...
export default class Level3Finish extends Phaser.Scene{
    ...
    create(data) {
        ...
        if (data.par.score < 32){
            bg = this.add.image(innerWidth*0.35, innerHeight/20, 'bg_l3f').setScale(0.4*stat.koeff, 0.4*stat.koeff).setOrigin(0);
            bg_width = bg.width*0.4*stat.koeff;
            bg_height = bg.height*0.4*stat.koeff;
            var comp = this.add.image(bg.x+bg_width*0.78, bg.y+bg_height*0.88, 'complete_l3f').setScale(0.1*stat.koeff, 0.1*stat.koeff).setInteractive().setTint(0x696969);
            stat.lvl3_score = this.score;
        }
        else{
            bg = this.add.image(innerWidth*0.35, innerHeight/20, 'bgsuccess_l3f').setScale(0.4*stat.koeff, 0.4*stat.koeff).setOrigin(0);
            bg_width = bg.width*0.4*stat.koeff;
            bg_height = bg.height*0.4*stat.koeff;
            var comp = this.add.image(bg.x+bg_width*0.78, bg.y+bg_height*0.88, 'complete_l3f').setScale(0.1*stat.koeff, 0.1*stat.koeff).setInteractive();
            stat.lvl3_score = this.score;
        ...
        var home = this.add.image(bg.x+bg_width*0.22, bg.y+bg_height*0.88, 'home_l3f').setScale(0.1*stat.koeff, 0.1*stat.koeff).setInteractive();
        var replay = this.add.image(bg.x+bg_width*0.5, bg.y+bg_height*0.88, 'replay_l3f').setScale(0.1*stat.koeff, 0.1*stat.koeff).setInteractive();
