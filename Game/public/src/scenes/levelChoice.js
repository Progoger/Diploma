...
export default class levelChoice extends Phaser.Scene {
    ...
    create() {
        ...
        if (stat.lvl1_completed) {
            var level2 = this.add.image(innerWidth/2, innerHeight/5*3, 'level2_lc').setScale(0.25*stat.koeff, 0.25*stat.koeff).setInteractive();
            ...
            this.add.image(innerWidth/2, innerHeight/5*3, 'block_lc').setScale(0.25*stat.koeff, 0.25*stat.koeff).setInteractive();
        }

        if (stat.lvl2_completed) {
            var level3 = this.add.image(innerWidth/2, innerHeight/5*4, 'level3_lc').setScale(0.25*stat.koeff, 0.25*stat.koeff).setInteractive();
            ...
            this.add.image(innerWidth/2, innerHeight/5*4, 'block_lc').setScale(0.25*stat.koeff, 0.25*stat.koeff).setInteractive();
        }
