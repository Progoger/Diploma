...
export default class Window extends Phaser.Scene{
    ...
    preload() {
        this.load.image(`bg_mw_${stat.active_level}_${stat.lvl2_active_cell}`, `src/assets/windows/bgs/${stat.active_level}/${stat.lvl2_active_cell}.png`);
    ...
    create(data) {
        ...
        var pay = null;
        var cross = this.add.image(bg.x+bg_width*0.97, bg.y*1.05, 'cross_mw').setScale(0.09*stat.koeff, 0.09*stat.koeff).setInteractive();
        if (data.type.split('_').length>1){
            var input = this.add.image(bg.x+bg_width*0.41, bg.y+bg_height*0.73, 'input_mw').setScale(0.22*stat.koeff, 0.22*stat.koeff);
            var textEntry = this.add.text(bg.x+bg_width*0.2, bg.y+bg_height*0.705, '', { font: `${58*stat.koeff}px Courier`, fill: '#000000' });
            pay = this.add.image(bg.x+bg_width*0.5, bg.y+bg_height*0.88, 'pay_mw').setInteractive().setScale(0.22*stat.koeff, 0.22*stat.koeff);
